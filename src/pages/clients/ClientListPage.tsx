...
export default function ClientListPage() {
  ...
  const handleFilterKeyPress = useCallback((e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      handleApplyFilters();
    }
  }, [handleApplyFilters]);
      ...
      <div className="card">
        ...
        <div className="card-footer flex flex-col sm:flex-row justify-between items-center space-y-2 sm:space-y-0 sm:space-x-4">
          ...
          <div className="text-sm text-gray-600">
            {hasAppliedFilters ? (
              <span>
                Filtros aplicados: {Object.entries(appliedFilters)
      ...
      {!isLoading && totalClients > 0 && (
        <div className="flex flex-col sm:flex-row justify-between items-center space-y-4 sm:space-y-0 p-4 bg-white rounded-xl shadow-sm border border-gray-200">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-2">
              <label htmlFor="limit-select" className="text-sm text-gray-600 hidden sm:block">Itens por página:</label>
              <select
                id="limit-select"
